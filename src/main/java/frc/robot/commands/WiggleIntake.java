...
public class WiggleIntake extends Command {
    ...
    public WiggleIntake(Intake intake) {
     ...
     @Override
     public void execute() {
        if((m_intake.isAtTarget(intakePos.WiggleIn.position) || WiggleTimer.get() >= IntakeConstants.wiggleTime) && isWiggleIn) {
             WiggleTimer.reset();
             WiggleTimer.start();
            m_intake.goToPosition(intakePos.WiggleOut);
                isWiggleIn = false;
            System.out.println("WiggleOut");
        } else if ((m_intake.isAtTarget(intakePos.WiggleOut.position) || WiggleTimer.get() >= IntakeConstants.wiggleTime) && !isWiggleIn) {
             WiggleTimer.reset();
             WiggleTimer.start();
            m_intake.goToPosition(intakePos.WiggleIn);
            System.out.println("WiggleIn");
                isWiggleIn = true;
        }
